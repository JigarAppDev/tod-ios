...
- (void)presentPanModal:(UIViewController<HWPanModalPresentable> *)viewControllerToPresent sourceView:(UIView *)sourceView sourceRect:(CGRect)rect completion:(void (^)(void))completion {
    ...
    /*if ([viewControllerToPresent isKindOfClass:[UINavigationController class]]) {
        NSLog(@"isKindOfClass:[UINavigationController");
        UINavigationController *nvc = (UINavigationController *)viewControllerToPresent;
        NSArray *viewControllers = nvc.viewControllers;
        UIViewController *rootViewController = [viewControllers firstObject];
        NSLog(@"nvc.topViewController => %@",rootViewController);
        rootViewController.hw_panModalPresentationDelegate = delegate;
    }else{
        viewControllerToPresent.hw_panModalPresentationDelegate = delegate;
    }*/
